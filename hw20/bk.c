...
static int _compare_func(const void* void_next, const void* void_curr) {		//Compare helper function
	...
	int current_value/*_freq*/ = (int)(((TreeNode*)void_curr) -> frequency);
	int compare_value/*_freq*/ = (int)(((TreeNode*)void_next) -> frequency);
	//int current_value_char = (int)(((TreeNode*)void_curr) -> character);
	//int compare_value_char = (int)(((TreeNode*)void_next) -> character);
	int result = 0;

	//if(current_value_freq == compare_value_freq) {								//If the frequencies are the same, return the difference between the two character values
	//	return (current_value_char - compare_value_char);
	//}
	//return (current_value_freq - compare_value_freq);							//Else, return the difference between the two frequencies
	//if(freq 1 = freq 2) {
	//	return char 1 - char 2
	//else return freq 1 - freq 2
