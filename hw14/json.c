...
Node* _insert_a_tail(Node* a_head, Node* new_node) {			//Helper function to move thr position to the end of the list (a_tail)
...
bool parse_list(Node** a_head, char const** a_pos) {
	...
	*a_pos = *a_pos + 1;
	...
	while(**a_pos != '\0') {										//Loops through the list and adds elements to the list
		...
		if(element_int_string_list.type == ELEMENT_INT) {
			Node* node_integer = malloc(sizeof(*node_integer));
			node_integer -> element = element_int_string_list;
			*a_head = _insert_a_tail(*a_head, node_integer);
		}
		if(element_int_string_list.type == ELEMENT_STRING) {
			Node* node_string = malloc(sizeof(*node_string));
			node_string -> element = element_int_string_list;
			*a_head = _insert_a_tail(*a_head, node_string);
		}
		//if(comma_num != 0) {
