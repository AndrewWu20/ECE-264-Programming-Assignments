...
static int _test_parse_list_one_int() {					//Tests a list with an integer inside
	...
	mu_check(is_success);
	if(is_success != 0) {
		mu_check(integer -> element.as_int == 5);
	}
...
static int _test_parse_list_neg_int() {					//Tests a list with a negative integer inside
	mu_start();
	//
	char const* input = "[-3]";
	char const* pos = input;
	...
	mu_check(is_success);
	if(is_success != 0) {
		mu_check(integer -> element.as_int == -3);
	}
	free_element(integer -> element);
	free(integer);
	//
...
//static void _test_print_list() {						//Tests printing a list
//	Element element;
...
int main(int argc, char* argv[]) {
	...
	mu_run(_test_parse_list_neg_int);
